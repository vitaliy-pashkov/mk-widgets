...
MKWidgets.Date = Class({
	...
	setMinMaxDates: function ()
		{
		if (typeof this.options.minDate === 'number')
			{
			this.options.minDate = moment().endOf('day').add(this.options.minDate, 'days');
			}
		else if (typeof this.options.minDate === 'string')
			{
			this.options.minDate = moment(this.options.minDate, this.options.locale.format)
			}
		...
		if (typeof this.options.maxDate === 'number')
			...
			{
			this.options.maxDate = moment(this.options.maxDate, this.options.locale.format)
	...
	renderCalendar: function (side)
		...
		for (var row = 0; row < 6; row++)
			...
			for (var col = 0; col < 7; col++)
				...
				//if (this.options.endDate != null && this.options.endDate < this.options.startDate && calendar[row][col] > this.options.endDate && calendar[row][col] < this.options.startDate)
				//	{
				//	classes.push('in-range');
				//	}
	...
	hoverDate: function (e)
		...
		if (!this.options.endDate && this.options.dateRange == undefined)
			{
			this.container.find('.calendar td').each(
				function (index, el)
				{
				...
				//else
				//	{
				//	if (dt.isAfter(date) && dt.isBefore(startDate))
				//		{
				//		$(el).addClass('in-range');
				//		}
				//	else
