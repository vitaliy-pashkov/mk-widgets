    ...
    init: function()
    {
        if(!this.options.jquery_container) {
            if(this.element.is(':container')) {
                ...
                // generate new
                this.options.jquery_element = uniqueId($('<span class="jqCron"></span>'), 'jqCron').insertAfter(this.element);
            }
            else {
...
MKWidgets.jqCronNS.jqCron = Class({
    ...
    init: function()
    {
        ...
        this.domObject.bind('cron:change', $.proxy(function(evt, value){
            if(!this.options.bind_to) return;
        ...
        this.selectorPeriod.element.bind('selector:change', $.proxy(function(e, value){
    ...
    newSelector: function(block, multiple, type)
        ...
        var selector = new MKWidgets.jqCronNS.Selector(this, block, multiple, type);
            ...
            selector.element.bind('selector:change', $.proxy(function(){
    ...
    setCron: function(str)
        ...
        try
            ...
            var items = str.split(' ');
            if (items.length != 6) this.error(this.getText('error2'));
            if(mask == '******')
                {
...
MKWidgets.jqCronNS.Selector = Class({
    ...
    add: function(key, value)
        {
        if(!(value+'').match(/^[0-9]+$/)) this.hasNumericTexts = false;
